---
title: Demo
tableOfContents: false
prev: false
next:
  link: /getting-started/
  label: Getting Started
---

import { PackageManagers } from 'starlight-package-managers'
import { Preview } from '@hideoo/starlight-plugins-docs-components'

:::tip[Did you know?]
This entire documentation site is using `starlight-package-managers` to display commands so do not hesitate to check out the various pages.
:::

After installing the package, you can use the component provided by `starlight-package-managers` in any MDX files of your Starlight documentation site.

<Preview>

```mdx title="src/content/docs/example.mdx"
import { PackageManagers } from 'starlight-package-managers'

<PackageManagers pkg="astro" />
```

<PackageManagers slot="preview" pkg="astro" />

</Preview>

You can also use the [compact variant](/usage/#variant) to merge the package manager tabs with the code frame.
Selections stay synced between both variants on the same page.

<PackageManagers
  type="create"
  pkg="astro@latest"
  args="--template starlight"
  variant="compact"
/>
